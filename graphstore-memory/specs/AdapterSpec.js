...
describe("adapter spec v2", function() {
	var m = null;
	...
	it("should get a user", function(done) {
		m.startLoading().then(function(){
			m.addUser({foo:'bar'}).then(function( id ){
				m.getUser( id ).then( function( user ) {
	...
	it("should get all users", function(done) {
		m.startLoading().then(function(){
			m.addUser({foo:'bar'}).then(function( id ){
				m.getUsers().then( function( users ) {
	...
	it("should update a user", function(done) {
		m.startLoading().then(function(){
			m.addUser({foo:'bar'}).then(function( id ){
				m.updateUser( id, { foo:'baz' } ).then( function() {
					m.getUser( id ).then( function( user ) {
						expect( user ).not.toBe( null );
	...
	it("should delete a user", function(done) {
		m.startLoading().then(function(){
			m.addUser({foo:'bar'}).then(function( id ){
				m.deleteUser( id, { foo:'baz' } ).then( function() {
					m.getUsers().then( function( users ) {
						expect( users ).not.toBe( null );
	...
	it("should get a product", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( id ){
				m.getProduct( id ).then(function( product ){
					expect( product ).not.toBe(null);
					expect( product.foo ).toBe( 'bar' );
					done();
				});
			});
		});
	...
	it("should get all product", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( id ){
				m.getProducts().then(function( products ){
					expect( products ).not.toBe(null);
					expect( products.length ).toBe( 1 );
					expect( products[0].foo ).toBe( 'bar' );
					done();
				});
			});
		});
	...
	it("should update a product", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( id ){
				m.updateProduct( id, {foo:'baz'} ).then(function( product ){
					m.getProduct( id ).then(function( product ){
						expect( product ).not.toBe( null );
						expect( product.foo ).toBe( 'baz' );
						done();
					});
				});
			});
	...
	it("should delete a product", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( id ){
				m.deleteProduct( id ).then(function(){
					m.getProducts().then(function( products ){
						expect( products ).not.toBe(null);
	...
	it("should get all affinities", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( product_id ){
				expect(product_id).not.toBe(null);
				...
				m.addUser({foo:'bar'}).then(function( user_id ){
					...
					m.addAffinity(user_id,product_id,'like').then(function( id ){
						m.getAffinities().then( function( affinities ) {
							expect(affinities).not.toBe(null);
							expect(affinities.length).toBe(1);
							expect(affinities[0].user_id).toBe(user_id);
							expect(affinities[0].product_id).toBe(product_id);
							expect(affinities[0].relation).toBe('like');
							done();
						} );
					});
				});
	...
	it("should delete affinities", function(done) {
		m.startLoading().then(function(){
			m.addProduct({foo:'bar'}).then(function( product_id ){
				expect(product_id).not.toBe(null);
				...
				m.addUser({foo:'bar'}).then(function( user_id ){
					...
					m.addAffinity(user_id,product_id,'like').then(function( id ){
						m.deleteAffinity(user_id,product_id,'like').then(function( ){
							m.getAffinities().then( function( affinities ) {
								expect(affinities).not.toBe(null);
								expect(affinities.length).toBe(0);
								done();
							} );
						});
					});
